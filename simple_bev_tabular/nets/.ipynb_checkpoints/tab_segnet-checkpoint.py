...
class Tab_Segnet(nn.Module):
    ...
    def forward(self, rgb_camXs, pix_T_cams, cam0_T_camXs, vox_util, rad_data_1, rad_data_2, device):
        ...
        pred = torch.zeros((rad_data_1.shape[0], rad_data_1.shape[1], 8)).to(device)
        ...
        if self.use_radar:
            ...
            if not self.use_metaradar:
                ...
                rad_bev_ = torch.sum(rad_occ_mem0, 1)
                ...
                # ax1.grid(False)
                # ax1.imshow(rad_bev_show_1, interpolation='none')
                # ax2 = fig.add_subplot(2,1,2)
                # ax2.grid(False)
                # ax2.imshow(rad_bev_show_2, interpolation='none')
                # fig.show()
