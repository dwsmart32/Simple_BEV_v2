...
def run_model(model, loss_fn, d, device='cuda:0', sw=None):
    ...
    xyz_rad = rad_data[:,:,:3]
    meta_rad = rad_data[:,:,3:]
    #print("xyz_rad is :", xyz_rad)
    #print("meta_rad is :", meta_rad)
...
def main(
        ...
        data_dir='/mnt/d/jaehyun/HVDetFusion/data/nuScenes',
    ...
    model = Tab_Segnet(Z, Y, X, vox_util, use_radar=use_radar, use_lidar=use_lidar, use_metaradar=use_metaradar, do_rgbcompress=do_rgbcompress, encoder_type=encoder_type, rand_flip=rand_flip, dim_feat=dim_feat, depth=depth, head=head, shared_divisor=shared_divisor)
